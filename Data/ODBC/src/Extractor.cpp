...
bool Extractor::extractBoundImpl<std::string>(std::size_t pos, std::string& val)
{
	...
	char* sp = AnyCast<char*>((*_pPreparator)[pos]);
...
{
	if (isNull(pos)) return false;
	SQL_DATE_STRUCT& ds = *AnyCast<SQL_DATE_STRUCT>(&((*_pPreparator)[pos]));
...
{
	std::vector<SQL_DATE_STRUCT>& ds = RefAnyCast<std::vector<SQL_DATE_STRUCT> >((*_pPreparator)[pos]);
	Utility::dateSync(val, ds);
	return true;
}
...
bool Extractor::extractBoundImpl<Poco::Data::Time>(std::size_t pos, Poco::Data::Time& val)
{
	...
	std::size_t dataSize = _pPreparator->actualDataSize(pos);
	checkDataSize(dataSize);
	SQL_TIME_STRUCT& ts = *AnyCast<SQL_TIME_STRUCT>(&(*_pPreparator)[pos]);
...
{
	std::vector<SQL_TIME_STRUCT>& ds = RefAnyCast<std::vector<SQL_TIME_STRUCT> >((*_pPreparator)[pos]);
...
bool Extractor::extractBoundImpl<Poco::DateTime>(std::size_t pos, Poco::DateTime& val)
{
	...
	std::size_t dataSize = _pPreparator->actualDataSize(pos);
	checkDataSize(dataSize);
	SQL_TIMESTAMP_STRUCT& tss = *AnyCast<SQL_TIMESTAMP_STRUCT>(&(*_pPreparator)[pos]);
...
{
	std::vector<SQL_TIMESTAMP_STRUCT>& ds = RefAnyCast<std::vector<SQL_TIMESTAMP_STRUCT> >((*_pPreparator)[pos]);
	...
	std::vector<bool>& val)
{
	...
	bool** p = AnyCast<bool*>(&(*_pPreparator)[pos]);
