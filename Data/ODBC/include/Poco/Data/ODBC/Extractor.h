...
private:
	...
	bool extractBoundImpl(std::size_t pos, T& val)
	{
		...
		poco_assert_dbg (typeid(T) == (*_pPreparator)[pos].type());
		val = *AnyCast<T>(&(*_pPreparator)[pos]); 
		return true;
	}
	...
	bool extractBoundImplContainer(std::size_t pos, C& val)
	{
		...
		poco_assert_dbg (typeid(std::vector<Type>) == (*_pPreparator)[pos].type());
		std::vector<Type>& v = RefAnyCast<std::vector<Type> >((*_pPreparator)[pos]);
	...
	bool extractBoundImplContainerString(std::size_t pos, C& values)
	{
		...
		CharType** pc = AnyCast<CharType*>(&((*_pPreparator)[pos]));
		poco_assert_dbg (pc);
	...
	bool extractBoundImplLOB(std::size_t pos, Poco::Data::LOB<T>& val)
	{
		...
		T* sp = AnyCast<T*>((*_pPreparator)[pos]);
